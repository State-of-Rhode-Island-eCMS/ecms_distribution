...
## Managing ACSF sites and backups

Site backups and post-upgrade health checks are handled by the eCMS ops
console app in `deployment-scripts/`, a Symfony Console application (see
`deployment-scripts/README.md`). It replaces the old standalone
`backup.php`/`post-upgrade-test.php` scripts, reads API credentials from
environment variables (never hardcode a key in a file), and defaults to the
stage environment — production is only touched when `--prod` is passed
explicitly.
